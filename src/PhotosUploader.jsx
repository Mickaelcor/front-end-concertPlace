...
export default function PhotosUploader({ addedPhotos, onChange }) {
    ...
    // Fonction pour l'ajout de photo par lien 
    ...
    return (
        <>
            <div className="flex gap-2">
                ...
                <button onClick={addPhotoByLink} className="bg-gray-200 px-4 rounded-2xl">Ajoutez&nbsp;les&nbsp;photos</button>
            </div>

            <div className="mt-2 grid gap-2 grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
                {addedPhotos.length > 0 && addedPhotos.map(link => (
                    <div className="h-32 flex relative" key={link}>
                        <img className="rounded-2xl w-full object-cover" src={"https://concertplace-backend.onrender.com/uploads/" + link} alt="" />
                        {/*Rajout d'une poubelle sur la photo upload pour la supprimer*/}
                        ...
                        {/*Rajout d'une etoile sur la photo upload pour la mettre en photo principale*/}
                        <button onClick={ev => selectAsMainPhoto(ev, link)} className="cursor-pointer absolute bottom-1 left-1 text-white bg-black bg-opacity-50 rounded-2xl py-2 px-3">
                            {/*A travers le link on compare pour savoir quelle photo a le premier lien pour afficher une etoile pleine sur la main photo */}
                            {link === addedPhotos[0] && (
                ...
                <label className="h-32 cursor-pointer flex items-center gap-1 justify-center border-2 border-black bg-transparent rounded-2xl p-2 text-2xl text-gray-600">
